...
namespace br {
	...
	namespace detail {
		...
		template <typename T>
		inline vec<T> alloc(vec<T> v, size_t count) {
	...
	constexpr vec<T> emplace(vec<T> v, Ts... args) {
	...
	template <typename T>
	constexpr vec<T> push(vec<T> v, T x) {
		return emplace(v, x);
	}

	template <typename T>
	constexpr vec<T> pop(vec<T> v, size_t n = 1) {
		BR_ASSERT(n > 0);
		v.used -= n;
		return v;
	}
	...
	template <typename T>
	constexpr vec<T> destroy_vec(vec<T> v) {
		free(v.data);
		v.data = nullptr;
		return v;
	}
